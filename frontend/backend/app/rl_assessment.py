...
async def execute_code(request: CodeExecutionRequest):
    ...
    For hackathon: Mock execution with simple validation.
    Production: Use secure sandbox (Judge0, Piston, etc.)
    """
    ...
    try:
        # Simple validation
        has_function = "def " in request.code or "function " in request.code
        has_return = "return" in request.code
        has_logic = any(keyword in request.code for keyword in ["if", "for", "while"])

        # Mock test results
        if has_function and has_return and has_logic:
            return {
                "success": True,
            ...
            return {
                "success": False,
                "output": "Test 1: ✓\nTest 2: ✗",
                "error": "AssertionError: Expected [4,7,2,9,6,3,1] but got [4,2,7,1,3,6,9]",
                "executionTime": 38,
                "testsPassed": 1,
                "testsTotal": 2,
            }
            ...
            return {
                "success": False,
                "error": "SyntaxError: Function 'invertTree' is not defined or has no return statement",
                "testsPassed": 0,
                "testsTotal": 2,
            }
